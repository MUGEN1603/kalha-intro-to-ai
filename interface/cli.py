...
def display_board(state):
    ...
    Board Layout:
        ...
        [S0]  [p0][p1][p2][p3][p4][p5]  <- Player 0's side
    """
    ...
    print()
    print("=" * 56)
    print(f"    [S1={state.stores[1]:2d}]  {p1_pits}  <- Player 1")
    print(f"             pit: 5    4    3    2    1    0")
    print(f"             pit: 0    1    2    3    4    5")
    print(f"    [S0={state.stores[0]:2d}]  {p0_pits}  <- Player 0")
    print("=" * 56)
...
def human_vs_ai(ai_player=1, depth=8):
    ...
    print(f"You are Player {1 - ai_player}, AI is Player {ai_player}")
    ...
    while not terminal_test(state):
        ...
        if current != ai_player:
            # Human turn
            legal = actions(state)
            print(f"Your turn (Player {current}). Legal pits: {legal}")
            while True:
                try:
    ...
    print(f"\nFinal Score -- P0: {state.stores[0]}, P1: {state.stores[1]}")
    print("Draw!" if winner == -1 else f"Player {winner} wins!")
...
if __name__ == "__main__":
    print("=== KALAHA AI ===")
    print("You are Player 0 (bottom)")
    print("AI is Player 1 (top)")
    print()
    human_vs_ai()
